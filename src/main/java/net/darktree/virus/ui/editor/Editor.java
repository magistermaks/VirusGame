...
public class Editor implements DrawContext {
	...
	private int getOptionCount() {
		...
		if (selected instanceof GenomeCell) {
			...
			if (type == EditType.CODON_ARGS)
				return selectedCodon == -1 ? 0 : cell.getGenome().codons.get(selectedCodon).getArgs().length;
	...
	public boolean isDivineControlAvailable(int id) {
		...
		if (id == 2 || id == 3)
			return (selected != null && selected.getType() != CellType.LOCKED && selected.getType() != CellType.CLEANER);
		...
		if (id == 6) return (selected == null || selected.getType() != CellType.CLEANER);
		return true;
	}
